...
const Skills = () => {
  ...
  return (
    <section id="skills" className="py-28 bg-white dark:bg-slate-800 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <span className="section-label mb-4 inline-flex">{t('skills.subtitle')}</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {skillCategories.map((category, idx) => (
            <motion.div
              ...
              className="bg-white dark:bg-slate-700/40 p-7 rounded-2xl border border-slate-100 dark:border-slate-700 card-glow"
            >
              {/* Header */}
              <div className="flex items-center gap-3 mb-6">
                <div className={`p-2.5 rounded-xl border ${category.iconBg} ${category.color}`}>
                  {category.icon}
                </div>
                <h3 className="font-display text-base font-bold text-slate-800 dark:text-white leading-tight">
                  {category.title}
                </h3>
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                {category.skills.map((skill, sIdx) => (
                  <span
                    key={sIdx}
                    className={`px-3 py-1.5 text-xs font-semibold rounded-lg border font-mono transition-colors ${category.tagBg}`}
                  >
                    {skill}
                  </span>
                ))}
